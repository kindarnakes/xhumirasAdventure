...
public class CharacterController2D : MonoBehaviour
{
    ...
    private float pixelsToMove = -194;
    ...
    public void Move(float move, bool jump, bool dash)
    {
        if (canMove)
        {
            ...
            if (isDashing)
            {
                ...
                StartCoroutine(MakeInvincible(0.6f));
                m_Rigidbody2D.velocity = new Vector2(transform.localScale.x * m_DashForce * -1, 0);
            }
    ...
    public void heal(float lifeToHeal){
        
            float actualLife = life;
            actualLife += lifeToHeal; 
            life = actualLife>dataConserved.DATA.maxLife?dataConserved.DATA.maxLife:actualLife;
